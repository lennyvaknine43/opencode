...
export namespace SessionPrompt {
  ...
  export const loop = fn(LoopInput, async (input) => {
    ...
    let hit: MessageV2.WithParts | undefined
    try {
      ...
      while (true) {
        ...
        const next = msgs.findLast((msg) => msg.info.role === "assistant")
        if (next) hit = next
        ...
        hit = { info: processor.message, parts: [] }
      ...
      try {
        ...
        if (abort.aborted && NotFoundError.isInstance(err) && hit) return hit
        throw err
      }
